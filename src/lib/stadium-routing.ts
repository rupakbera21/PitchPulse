...
export const isStandClosed = (zoneId: string, crowdData: any) => {
  const zone = crowdData?.zones?.find((z: any) => z.id === zoneId);
  if (zone?.is_closed) return true;
  
  if (zoneId.includes('_n') && crowdData?.zones?.find((z: any) => z.id === 'gate_n')?.is_closed) return true;
  if (zoneId.includes('_s') && crowdData?.zones?.find((z: any) => z.id === 'gate_s')?.is_closed) return true;
  if (zoneId.includes('_e') && crowdData?.zones?.find((z: any) => z.id === 'gate_e')?.is_closed) return true;
  if (zoneId.includes('_w') && crowdData?.zones?.find((z: any) => z.id === 'gate_w')?.is_closed) return true;
...
export const getBorderRadius = (zone: any) => {
